        ...
        render: function() {
            ...
            if(this.state.editing === 'name') {
                name = React.DOM.form({ id: 'name', onSubmit: this.handleUpdate('name')},
                    React.DOM.input({ autoFocus: true, type: 'text', placeholder: 'Name', ref: 'name', className: 'name', required: true, defaultValue: this.props.contact.name}),
            ...
            if(this.state.editing === 'description' || this.props.contact.description.trim().length === 0) {
                ...
                description = React.DOM.div(null, 
                    React.DOM.h3({ className: 'description' }, 'Description: '),
                    React.DOM.p({ ref: 'description', className: 'description'}, (this.props.contact.description.length<1)?'Insert Description':this.props.contact.description)
                );
            }
